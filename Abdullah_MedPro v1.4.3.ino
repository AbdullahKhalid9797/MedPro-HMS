...
#define SampleRate 200 // Increased for better signal quality
#define varAvgSize 4
#define thsIRFinger 50000  // Increased to ensure stronger finger signal
...
void resetI2C() {
  if (bolDebug) Serial.println("Resetting I2C bus...");
  Wire.end();
  ...
  if (bolDebug) Serial.println("I2C bus reset complete.");
}
...
void setup() {
  ...
  byte irLEDBr = 100; // Increased for better signal
  byte byteSmplAvg = 4; // Reduced to improve responsiveness
  byte irLEDMode = 2;
  int intPulseWidth = 411; // Reduced for better signal quality
  int intRange = 8192; // Adjusted for sensitivity
...
void getTemp(DeviceAddress devAdd) {
  if (bolDebug) Serial.println("Reading DS18B20...");
...
void readMLX90614() {
  if (bolDebug) Serial.println("Reading MLX90614...");
  ...
  if (error != 0) {
    ...
    if (error != 0) {
      if (bolDebug) Serial.println("MLX90614 I2C error persists after reset.");
      return;
    }
  }
...
void readMAX30102() {
  ...
  // Add 25 new samples with timeout
  for (byte i = 75; i < lenBuffer; i++) {
    uint32_t startTime = millis();
    while (!pulOxym.available()) {
      if (millis() - startTime > MAX30102_TIMEOUT) {
        if (bolDebug) Serial.println("MAX30102 timeout waiting for data.");
        return; // Exit if timeout occurs
      }
      pulOxym.check();
      handleButtons(); // Check buttons to allow stopping/switching
      if (!isReading || (currentState != SENSOR_MAX30102 && currentState != SENSOR_AUTO)) {
        if (bolDebug) Serial.println("MAX30102 reading interrupted by button press.");
        return; // Exit if readings stopped or state changed
      }
      delay(1);
    }
  ...
  if (irMax - irMin < 1000) { // Check for low signal variation (indicates noise)
    if (bolDebug && millis() - intLstRep >= RespInterval) {
      Serial.println("MAX30102 signal too weak or noisy.");
      Serial.print("IR Min: ");
      Serial.print(irMin);
      Serial.print(", IR Max: ");
...
void readAutoMode() {
  if (bolDebug) Serial.println("Reading Auto Mode...");
  // Read DS18B20
  senDS18B20.begin(); // Reinitialize to ensure readiness
  senDS18B20.requestTemperatures();
  for (int i = 0; i < devDS18B20; i++) {
...
void manageSensorPower() {
  ...
  pulOxym.shutDown();
  if (bolDebug) Serial.println("MAX30102 powered off.");
  ...
  if (currentState == SENSOR_MAX30102 || currentState == SENSOR_AUTO) {
    ...
    if (bolDebug) Serial.println("MAX30102 powered on.");
  }
  // DS18B20 and MLX90614 do not require explicit power management
  if (currentState == SENSOR_DS18B20) {
    if (bolDebug) Serial.println("DS18B20 active.");
  }
  if (currentState == SENSOR_MLX90614) {
    if (bolDebug) Serial.println("MLX90614 active.");
  }
}
...
void handleButtons() {
  ...
  if (digitalRead(BUTTON_SELECT) == LOW) {
    ...
    if (!isReading) {
      ...
      if (bolDebug) Serial.println("All sensors stopped.");
    } else {
