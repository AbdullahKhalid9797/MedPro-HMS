/*
  ...
  - Ver 1.3 Added Support to capture Temperature value from MLX90614 Contactless IR Temperature Sensor
*/
#include <Wire.h>
...
void setup() {
  ...
  Wire.begin(23,22); // Initialize MAX30102 on SDA 23, SCL 22 instead of default I2C Channel (SDA 21, SCL 22)
  senDS18B20.begin(); // Initialize DS18B20 sensor
  if (!senMLX.begin()) { // Initialize MLX90614 IR sensor on I2C
    if(bolDebug) Serial.println("Error connecting to MLX sensor. Check wiring.");
    while (1);
  }; 
  if(bolDebug) Serial.println("Emissivity = "); Serial.println(senMLX.readEmissivity());
...
void loop() {
  ...
  if(bolDebug){
    ...
    Serial.print(senMLX.readAmbientTempC());  
    Serial.print("oC/"); Serial.print(senMLX.readAmbientTempF());
    Serial.print("oF\tObject = "); Serial.print(senMLX.readObjectTempC());
    Serial.print("oC/"); Serial.print(senMLX.readObjectTempF()); Serial.println("oF");
  }
